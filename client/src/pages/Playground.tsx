      ...
      <div className={`relative aspect-[4/3] bg-gradient-to-br ${CATEGORY_GRADIENTS[item.category] ?? "from-muted to-muted/50"} overflow-hidden`}>
        {item.imageUrl ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-5xl font-black opacity-5 text-foreground select-none">
              {item.title.charAt(0)}
            </div>
          </div>
        )}
...
export default function Playground() {
  ...
  const { data: items = [], isLoading } = useQuery<PlaygroundItem[]>({
      ...
      <div className="pt-16">
        ...
        <section className="mx-auto max-w-7xl px-6 lg:px-8 py-20">
          <div className="flex items-center gap-2 mb-6">
            <span className="text-xs bg-muted border border-border rounded-full px-3 py-1 text-muted-foreground">
              Experiments & Exploration
            </span>
          </div>
          <h1 className="text-5xl lg:text-6xl font-black text-foreground mb-4">Design Playground</h1>
          <p className="text-lg text-muted-foreground max-w-2xl leading-relaxed">
            A collection of experiments, daily UI challenges, and creative explorations. Not every idea makes it to production, but each one teaches something valuable.
          </p>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-6 lg:px-8 pb-20">
          <div className="rounded-3xl border border-border bg-muted/30 p-12 text-center">
            <h2 className="text-3xl font-black text-foreground mb-3">Want to see more?</h2>
            <p className="text-muted-foreground mb-8">Follow me on Dribbble and Behance for daily design inspiration</p>
