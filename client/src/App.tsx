...
import AdminProcessPage from "@/pages/admin/ProcessPage";
...
function AdminGuard({ component: Component }: { component: React.ComponentType }) {
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-background flex items-center justify-center"><div className="w-8 h-8 border-2 border-foreground border-t-transparent rounded-full animate-spin" /></div>;
  }
...
function Router() {
  return (
    <Switch>
      ...
      <Route path="/admin/process-page" component={() => <AdminGuard component={AdminProcessPage} />} />
